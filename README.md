...
<h3 align="center">读完了依赖～我很快就离开～</h3>
...
foo(bar() :<, buz());
```
Then remove your brain. The only way to implement this is to use gnu extension `({ ... })` or even something like in-place ffi, and this is not a good idea. C spec didn't defined the order of evaluation of bar() and buz(), and if bar() allocates a resource, will freeing it in :< be conflict with foo()'s logic?     
